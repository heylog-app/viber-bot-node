...
ViberClient.prototype.setWebhook = function (url, isInline) {
	this._logger.info(`Sending 'setWebhook' request for url: ${url}, isInline: ${isInline}`);
...
ViberClient.prototype.sendMessage = function (optionalReceiver, messageType, messageData, optionalTrackingData, optionalKeyboard, optionalChatId, optionalMinApiVersion) {
	...
	this._logger.debug(`Sending ${messageType} message to viber user '${optionalReceiver}' with data ${messageData}`);
...
ViberClient.prototype.postToPublicChat = function (senderProfile, messageType, messageData, optionalMinApiVersion) {
	...
	this._logger.debug(`Sending ${messageType} message to public chat as viber user '${senderProfile.id}' with data ${JSON.stringify(messageData)}`);
...
ViberClient.prototype._sendRequest = function (endpoint, data) {
	...
	this._logger.debug(`Opening request to url: '${url}' with data ${JSON.stringify(data) }`);
...
ViberClient.prototype._readData = function (data, resolve, reject) {
	if (data.status != SUCCESSFUL_REQUEST_STATUS) {
		this._logger.error(`Response error ${JSON.stringify(data)}`);
		return reject(data);
	}
	this._logger.debug(`Response data: ${JSON.stringify(data)}`);
	return resolve(data);
};

ViberClient.prototype._requestEnded = function (err, reject) {
	if (err) {
		this._logger.error(`Request ended with an error ${JSON.stringify(err)}`);
		return reject(err);
	}
